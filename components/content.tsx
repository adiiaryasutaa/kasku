...
import { Calendar, CreditCard, Wallet, DollarSign, TrendingUp, AlertCircle } from "lucide-react"
    ...
    async function loadFinancialSummary() {
      try {
        ...
        const monthlyIncome = transactions
          .filter((tx) => tx.type === "income" && tx.status === "completed" && tx.createdAt >= firstDayOfMonth)
          .reduce((sum, tx) => sum + tx.amount, 0)

        // Calculate monthly expenses
        const monthlyExpenses = transactions
          .filter((tx) => tx.type === "expense" && tx.status === "completed" && tx.createdAt >= firstDayOfMonth)
          .reduce((sum, tx) => sum + tx.amount, 0)
        ...
        const budgetUtilization = totalBudget > 0 ? Math.round((monthlyExpenses / totalBudget) * 100) : 0
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white dark:bg-[#0F0F12] rounded-xl p-4 flex items-center border border-gray-200 dark:border-[#1F1F23]">
          ...
          <div>
            ...
            <h3 className="text-xl font-bold text-gray-900 dark:text-white">{summary.budgetUtilization}%</h3>
