...
import { FileText, Download, Calendar, TrendingUp, TrendingDown, ArrowRight, Printer } from "lucide-react"
  ...
  useEffect(() => {
    async function loadReportData() {
      try {
        ...
        const largestIncome = incomeTransactions.length > 0 ? Math.max(...incomeTransactions.map((tx) => tx.amount)) : 0

        const largestExpense =
          expenseTransactions.length > 0 ? Math.max(...expenseTransactions.map((tx) => tx.amount)) : 0
        ...
        const monthCount = timeRange === "month" ? 1 : timeRange === "quarter" ? 3 : timeRange === "year" ? 12 : 12 // Default to 12 months for "all"
        ...
        transactions.forEach((tx) => {
          ...
          const monthIndex = months.findIndex(
            (month) => month.getMonth() === txDate.getMonth() && month.getFullYear() === txDate.getFullYear(),
          )
          ...
          if (monthIndex !== -1) {
            ...
            formattedMonthlyData[monthIndex].net =
              formattedMonthlyData[monthIndex].income - formattedMonthlyData[monthIndex].expenses
          }
        })
      ...
      <div className="space-y-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-gray-500 dark:text-gray-400 mt-1">Analyze your organization's financial performance</p>
          </div>
          ...
          <TabsContent value="overview" className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              <Card>
                ...
                <CardContent>
                  ...
                  <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">{summary.incomeCount} transactions</p>
                </CardContent>
              </Card>
              ...
              <Card>
                ...
                <CardContent>
                  ...
                  <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">{summary.expenseCount} transactions</p>
                </CardContent>
              </Card>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="text-2xl font-bold text-gray-900 dark:text-white">
                    {summary.totalExpenses > 0 ? (summary.totalIncome / summary.totalExpenses).toFixed(2) : "∞"}
                  </div>
                  <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                    {summary.totalExpenses > 0
                      ? `$${(summary.totalIncome / summary.totalExpenses).toFixed(2)} income per $1 expense`
                      ...
                      {monthlyData.map((monthData, index) => {
                        ...
                        return (
                          <div key={index} className="flex flex-col items-center flex-1 relative">
                            ...
                            >
                              <div className="invisible h-0">{monthData.income}</div>
                            </div>
                            ...
                            >
                              <div className="invisible h-0">{monthData.expenses}</div>
                            </div>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Categories with highest net income/expense</CardDescription>
              </CardHeader>
              <CardContent>
                {isLoading ? (
                  <div className="flex justify-center items-center p-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                      <h3 className="text-sm font-medium text-gray-900 dark:text-white mb-3">Top Income Categories</h3>
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1.5 mt-1">
                                  <div
                                    ...
                                    style={{
                                      width: `${Math.min(100, (category.income / summary.totalIncome) * 100)}%`,
                                    }}
                                  />
                                </div>
                              </div>
                              <div className="text-sm font-medium text-gray-900 dark:text-white">
                                ${category.income.toLocaleString(undefined, { maximumFractionDigits: 0 })}
                              </div>
                            </div>
                          ))}
                      </div>
                    ...
                    <div>
                      <h3 className="text-sm font-medium text-gray-900 dark:text-white mb-3">Top Expense Categories</h3>
                      <div className="space-y-3">
                        {categorySummary
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1.5 mt-1">
                                  <div
                                    ...
                                    style={{
                                      width: `${Math.min(100, (category.expenses / summary.totalExpenses) * 100)}%`,
                                    }}
                                  />
                                </div>
                              </div>
                              <div className="text-sm font-medium text-gray-900 dark:text-white">
                                ${category.expenses.toLocaleString(undefined, { maximumFractionDigits: 0 })}
                              </div>
                            </div>
                          ))}
                      </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Total: ${summary.totalIncome.toLocaleString(undefined, { maximumFractionDigits: 0 })}
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  {isLoading ? (
                    ...
                    <div className="space-y-4">
                      <div className="flex justify-between items-center">
                        <div>
                          <div className="text-sm font-medium text-gray-500 dark:text-gray-400">Largest Income</div>
                          <div className="text-lg font-bold text-gray-900 dark:text-white">
                            ${summary.largestIncome.toLocaleString(undefined, { maximumFractionDigits: 0 })}
                          </div>
                        </div>
                        <div>
                          <div className="text-sm font-medium text-gray-500 dark:text-gray-400">Average Income</div>
                          <div className="text-lg font-bold text-gray-900 dark:text-white">
                            $
                            {(summary.totalIncome / summary.incomeCount).toLocaleString(undefined, {
                      ...
                      <div className="pt-4">
                        <h4 className="text-sm font-medium text-gray-900 dark:text-white mb-3">Income by Category</h4>
                        <div className="space-y-3">
                          {categorySummary
                            .filter((cat) => cat.income > 0)
                            .sort((a, b) => b.income - a.income)
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Total: ${summary.totalExpenses.toLocaleString(undefined, { maximumFractionDigits: 0 })}
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  {isLoading ? (
                    ...
                    <div className="space-y-4">
                      <div className="flex justify-between items-center">
                        <div>
                          <div className="text-sm font-medium text-gray-500 dark:text-gray-400">Largest Expense</div>
                          <div className="text-lg font-bold text-gray-900 dark:text-white">
                            ${summary.largestExpense.toLocaleString(undefined, { maximumFractionDigits: 0 })}
                          </div>
                        </div>
                        <div>
                          <div className="text-sm font-medium text-gray-500 dark:text-gray-400">Average Expense</div>
                          <div className="text-lg font-bold text-gray-900 dark:text-white">
                            $
                            {(summary.totalExpenses / summary.expenseCount).toLocaleString(undefined, {
                              maximumFractionDigits: 0,
                            })}
                          </div>
                        </div>
                      ...
                      <div className="pt-4">
                        <h4 className="text-sm font-medium text-gray-900 dark:text-white mb-3">Expenses by Category</h4>
                        <div className="space-y-3">
                          {categorySummary
                            .filter((cat) => cat.expenses > 0)
                            .sort((a, b) => b.expenses - a.expenses)
                    ...
                    <div className="h-[300px] w-full">
                      ...
                      <div className="flex h-[250px] items-end gap-2 pt-6 relative">
                        ...
                        {monthlyData.map((monthData, index) => {
                          ...
                          const incomeHeight = maxValue > 0 ? (monthData.income / maxValue) * 100 : 0
                          const expenseHeight = maxValue > 0 ? (monthData.expenses / maxValue) * 100 : 0
                          const netHeight = maxValue > 0 ? (Math.abs(monthData.net) / maxValue) * 100 : 0
                            ...
                            <div key={index} className="flex flex-col items-center flex-1 relative">
                              ...
                              <div
                                className={`w-[30%] absolute bottom-[50%] mx-auto ${
                                  monthData.net >= 0 ? "bg-green-300 dark:bg-green-800" : "bg-red-300 dark:bg-red-800"
                                } rounded-sm`}
                                style={{
                                  height: `${Math.max(netHeight, 2)}%`,
                                  bottom: monthData.net >= 0 ? "50%" : `calc(50% - ${Math.max(netHeight, 2)}%)`,
                                }}
                              />
                      ...
                      <div className="flex justify-between mt-8">
                        ...
                        <div className="text-xs text-gray-500 dark:text-gray-400">Net income/loss by month</div>
                      </div>
                    </div>

                    <div className="grid grid-cols-3 gap-4 pt-4">
                      <div className="text-center">
                        <div className="text-sm font-medium text-gray-500 dark:text-gray-400">Profitable Months</div>
                        <div className="text-lg font-bold text-gray-900 dark:text-white">
                          {monthlyData.filter((m) => m.net > 0).length}
                        </div>
                      </div>
                      <div className="text-center">
                        <div className="text-sm font-medium text-gray-500 dark:text-gray-400">Loss Months</div>
                        <div className="text-lg font-bold text-gray-900 dark:text-white">
                          {monthlyData.filter((m) => m.net < 0).length}
                        </div>
                      </div>
                      <div className="text-center">
                        <div className="text-sm font-medium text-gray-500 dark:text-gray-400">Break-even Months</div>
                        <div className="text-lg font-bold text-gray-900 dark:text-white">
                          {monthlyData.filter((m) => m.net === 0).length}
                        </div>
                      </div>
                  ...
                  <div className="space-y-6">
                      ...
                      <table className="w-full">
                        ...
                        <tbody>
                          {categorySummary.map((category) => (
                            <tr key={category.id} className="border-b border-gray-100 dark:border-gray-800">
                              ...
                              >
                                ${Math.abs(category.net).toLocaleString(undefined, { maximumFractionDigits: 0 })}
                        ...
                        <tfoot>
                          <tr className="border-t border-gray-200 dark:border-gray-700">
                            <td className="py-3 px-2 text-sm font-medium text-gray-900 dark:text-white">Total</td>
                            ...
                            <td className="py-3 px-2 text-right text-sm font-medium text-red-600 dark:text-red-400">
                              ${summary.totalExpenses.toLocaleString(undefined, { maximumFractionDigits: 0 })}
                            </td>
                            <td
                              className={`py-3 px-2 text-right text-sm font-medium ${
                                summary.netIncome >= 0
                            ...
                            >
                              ${Math.abs(summary.netIncome).toLocaleString(undefined, { maximumFractionDigits: 0 })}
                              {summary.netIncome >= 0 ? " profit" : " loss"}
                            </td>
                          </tr>
                        </tfoot>
                    ...
                    <div className="pt-4">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                  ...
                                  <div className="flex-1 min-w-0">
                                    ...
                                    <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1.5 mt-1">
                                      <div
                                        className="bg-green-500 h-1.5 rounded-full"
                                        style={{
                                          width: `${Math.min(100, (category.net / Math.max(...categorySummary.filter((c) => c.net > 0).map((c) => c.net))) * 100)}%`,
                                        }}
                                      />
                                    </div>
                                  </div>
                                  <div className="text-sm font-medium text-green-600 dark:text-green-400">
                                    +${category.net.toLocaleString(undefined, { maximumFractionDigits: 0 })}
                                  </div>
                                </div>
                              ))}
                          </div>
                        ...
                        {/* Loss Categories */}
                        <div>
                          <h5 className="text-xs font-medium text-gray-500 dark:text-gray-400 mb-3">Loss Categories</h5>
                          <div className="space-y-3">
                            {categorySummary
                              .filter((cat) => cat.net < 0)
                                  ...
                                  <div className="flex-1 min-w-0">
                                    ...
                                    <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1.5 mt-1">
                                      <div
                                        className="bg-red-500 h-1.5 rounded-full"
                                        style={{
                                          width: `${Math.min(100, (Math.abs(category.net) / Math.max(...categorySummary.filter((c) => c.net < 0).map((c) => Math.abs(c.net)))) * 100)}%`,
                                        }}
                                      />
                                    </div>
                                  </div>
